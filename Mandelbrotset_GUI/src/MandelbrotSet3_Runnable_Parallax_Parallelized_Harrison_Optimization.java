...
public class MandelbrotSet3_Runnable_Parallax_Parallelized_Harrison_Optimization implements Runnable{
	
	static final int NUM_THREADS = 4;
	static final int xResolution = 1920;
	static final int yResolution = 1080;
	...
	public static int main(double xCord, double yCord, double radius, int quality, int width, int height, int layerCount, String[] args) throws IOException, InterruptedException {
		...
		centerA = xCord;
		centerB = yCord;
		MandelbrotSet3_Runnable_Parallax_Parallelized_Harrison_Optimization.radius = radius;
		MandelbrotSet3_Runnable_Parallax_Parallelized_Harrison_Optimization.quality = quality;
		MandelbrotSet3_Runnable_Parallax_Parallelized_Harrison_Optimization.width = width;
		MandelbrotSet3_Runnable_Parallax_Parallelized_Harrison_Optimization.height = height;
		MandelbrotSet3_Runnable_Parallax_Parallelized_Harrison_Optimization.layerCount = layerCount;
		...
		for(int x = 0; x<NUM_THREADS; x++){
			Threads[x] = new Thread(MandelbrotSet3_Runnable_Parallax_Parallelized_Harrison_Optimization(x));
			Threads[x].start();
...
//		}
	...
	private static Runnable MandelbrotSet3_Runnable_Parallax_Parallelized_Harrison_Optimization(int id) {
		this.ID = id;
		return null;
	}
	
	
//	public MandelbrotSet3_Runnable_Parallax_Parallelized_Harrison_Optimization(int id){
	...
	public void run(){
		...
		for (double h = centerB + (radius*height)/longer; h >= goToHeight && countHeight < height - 1; h -= ((2*radius)/longer)){//*(NUM_THREADS)) {
			for (double w = centerA - (radius*width)/longer; w <= goToWidth && countWidth < width - 1; w += (2*radius)/longer) {
				//Mandelbrot Set
            	int k = MandelbrotSet3_Runnable_Parallax_Parallelized_Harrison_Optimization.Calculate_MandlebrotSet(new Complex(w, h), (quality));
            	//Julia Set
