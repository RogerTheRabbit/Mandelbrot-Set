...
public class Parallax extends Thread {
...
//		}
	...
	public Parallax(double xCord, double yCord, double radius, int quality, int width, int height, int layerCount){
		centerA = xCord;
		centerB = yCord;
		Parallax.radius = radius;
		Parallax.quality = quality;
		Parallax.width = width;
		Parallax.height = height;
		Parallax.layerCount = layerCount;
		
		main(null);
		
	}
	
	public Parallax(int id){
	...
	public void run(){
		...
		for (double h = centerB + (radius*height)/longer; h >= goToHeight && countHeight < height - 1; h -= ((2*radius)/longer)){//*(NUM_THREADS)) {
			for (double w = centerA - (radius*width)/longer; w <= goToWidth && countWidth < width - 1; w += (2*radius)/longer) {
				//Mandelbrot Set
            	int k = Parallax.Calculate_MandlebrotSet(new Complex(w, h), (quality));
            	//Julia Set
